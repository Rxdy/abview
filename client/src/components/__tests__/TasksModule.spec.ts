...
describe('TasksModule', () => {
  ...
  it('handles task list updates', async () => {
    mockTasksStore.loading = false
    mockTasksStore.error = null
    mockTasksStore.tasks = [
      { id: 1, taskListTitle: 'List 1', listColor: 'red', completed: false }
    ...
    expect(wrapper.findAllComponents({ name: 'TaskList' }).length).toBeGreaterThan(0)

    // Update tasks
    mockTasksStore.tasks = [
      { id: 1, taskListTitle: 'List 1', listColor: 'red', completed: false },
      { id: 2, taskListTitle: 'List 2', listColor: 'blue', completed: true }
    ]
