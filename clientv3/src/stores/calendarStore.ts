...
export const useCalendarStore = defineStore('calendar', {
  ...
  getters: {
    ...
    pastYearStats: (state) => {
      // Return stats for the past year (placeholder - would need actual implementation)
      return {};
    },
  },
  actions: {
...
function transformHorairesToEvents(horaires: any[]): any[] {
  ...
  for (let i = 0; i < 8; i++) {
    ...
    const dayName = ["sunday", "monday", "tuesday", "wednesday", "thursday", "friday", "saturday"][dayOfWeek] as string;
    ...
    horaires.forEach((person) => {
      ...
      if (person.type === "shift") {
        ...
        if (currentShift) {
          ...
          const dayExceptions = (currentShift.exceptions as any)?.[dayName] || currentShift.hours;
